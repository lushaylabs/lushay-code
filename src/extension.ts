...
`;
    ...
    const res = spawnSync(
        ...
        [
            ...
            `-Wall`,
            gowinCellsPath,
            ...(existsSync(gowinXtraCellsPath) ? [gowinXtraCellsPath] : []),
            vltConfigPath,
...
async function getConfigOverrides(): Promise<Record<string, string>> {
    ...
    return {
        yosys: config.Build.overrideYosysPath,
        nextpnr: config.Build.overrideNextpnrPath,
        openFPGALoader: config.Build.overrideOpenFpgaLoaderPath
    }
}
